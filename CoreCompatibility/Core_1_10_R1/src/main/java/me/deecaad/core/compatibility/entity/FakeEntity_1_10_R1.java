...
import static net.minecraft.server.v1_10_R1.PacketPlayOutEntity.PacketPlayOutEntityLook;
import static net.minecraft.server.v1_10_R1.PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook;

public class FakeEntity_1_10_R1 extends FakeEntity {

    static {
        if (ReflectionUtil.getMCVersion() != 10) {
            me.deecaad.core.MechanicsCore.debug.log(
                    LogLevel.ERROR,
                    "Loaded " + FakeEntity_1_10_R1.class + " when not using Minecraft 10",
                    new InternalError()
            );
        }
    }
    ...
    public FakeEntity_1_10_R1(@NotNull Location location, @NotNull EntityType type, @Nullable Object data) {
        super(location, type);
                ...
                case FALLING_BLOCK:
                    if (data.getClass() == Material.class) {
                        entity = new EntityFallingBlock(world.getHandle(), x, y, z,
                                block = net.minecraft.server.v1_10_R1.Block.getByCombinedId(((Material) data).getId()));
                    } else {
