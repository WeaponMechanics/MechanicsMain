...
import static net.minecraft.server.v1_14_R1.PacketPlayOutEntity.PacketPlayOutEntityLook;
import static net.minecraft.server.v1_14_R1.PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook;

public class FakeEntity_1_14_R1 extends FakeEntity {

    static {
        if (ReflectionUtil.getMCVersion() != 14) {
            me.deecaad.core.MechanicsCore.debug.log(
                LogLevel.ERROR,
                "Loaded " + FakeEntity_1_14_R1.class + " when not using Minecraft 14",
                new InternalError());
        }
    }
    ...
    public FakeEntity_1_14_R1(@NotNull Location location, @NotNull EntityType type, @Nullable Object data) {
        super(location, type);
        ...
        if (data != null) {
            entity = switch (type) {
                ...
                case FALLING_BLOCK -> new EntityFallingBlock(world.getHandle(), x, y, z, block = (data.getClass() == Material.class
                    ? ((CraftBlockData) ((Material) data).createBlockData()).getState()
                    : ((CraftBlockState) data).getHandle()));
                case FIREWORK -> new EntityFireworks(world.getHandle(), item = CraftItemStack.asNMSCopy((org.bukkit.inventory.ItemStack) data), x, y, z, true);
