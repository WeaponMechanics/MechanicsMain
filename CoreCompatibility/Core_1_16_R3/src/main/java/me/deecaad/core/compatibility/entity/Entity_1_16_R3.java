...
public class Entity_1_16_R3 implements EntityCompatibility {
    ...
    public Object getSpawnPacket(Object entity) {
        if (!(entity instanceof Entity)) {
            throw new IllegalArgumentException("Given Object must be 1_16_R3 Entity!");
        }
    ...
    private FallingBlockWrapper createFallingBlock(Location loc, IBlockData data, Vector motion, int maxTicks) {
        ...
        EntityFallingBlock block = new EntityFallingBlock(world, loc.getX(), loc.getY(), loc.getZ(), data) {
            ...
            private Vec3D getCollisionVector(Vec3D vec3d) {
                ...
                Vec3D vec3d1 = vec3d.g() == 0.0D ? vec3d : Entity.a(this, vec3d, axisalignedbb, this.world, voxelshapecollision, streamaccumulator);
                ...
                if (this.G > 0.0F && flag3 && (flag || flag2)) {
                    Vec3D vec3d2 = Entity.a(this, new Vec3D(vec3d.x, (double)this.G, vec3d.z), axisalignedbb, this.world, voxelshapecollision, streamaccumulator);
                    Vec3D vec3d3 = Entity.a(this, new Vec3D(0.0D, (double)this.G, 0.0D), axisalignedbb.b(vec3d.x, 0.0D, vec3d.z), this.world, voxelshapecollision, streamaccumulator);
                    if (vec3d3.y < (double)this.G) {
                        Vec3D vec3d4 = Entity.a(this, new Vec3D(vec3d.x, 0.0D, vec3d.z), axisalignedbb.c(vec3d3), this.world, voxelshapecollision, streamaccumulator).e(vec3d3);
                        if (Entity.c(vec3d4) > Entity.c(vec3d2)) {
                            vec3d2 = vec3d4;
                        }
                    }

                    if (Entity.c(vec3d2) > Entity.c(vec3d1)) {
                        return vec3d2.e(Entity.a(this, new Vec3D(0.0D, -vec3d2.y + vec3d.y, 0.0D), axisalignedbb.c(vec3d2), this.world, voxelshapecollision, streamaccumulator));
                    }
                }
