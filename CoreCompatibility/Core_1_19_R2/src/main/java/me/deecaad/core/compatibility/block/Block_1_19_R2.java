...
public class Block_1_19_R2 implements BlockCompatibility {
    ...
    static {
        ...
        if (ReflectionUtil.getMCVersion() != 19) {
            MechanicsCore.debug.log(
                    LogLevel.ERROR,
                    "Loaded " + Block_1_19_R2.class + " when not using Minecraft 19",
                    new InternalError()
            );
        }
    }
    ...
    private ClientboundSectionBlocksUpdatePacket getMultiBlockMaskPacket(List<Block> blocks, @Nullable BlockState mask) {
        ...
        ClientboundSectionBlocksUpdatePacket packet = new ClientboundSectionBlocksUpdatePacket(SectionPos.of(position), new ShortArraySet(0), data, false);
