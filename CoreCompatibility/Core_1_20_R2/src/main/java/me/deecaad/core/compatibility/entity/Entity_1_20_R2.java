...
import me.deecaad.core.compatibility.equipevent.NonNullList_1_20_R2;
...
public class Entity_1_20_R2 implements EntityCompatibility {

    static {
        if (ReflectionUtil.getMCVersion() != 20) {
            MechanicsCore.debug.log(
                    LogLevel.ERROR,
                    "Loaded " + Entity_1_20_R2.class + " when not using Minecraft 20",
                    new InternalError()
            );
        }
    }
    ...
    public List generateNonNullList(int size, TriIntConsumer<org.bukkit.inventory.ItemStack, org.bukkit.inventory.ItemStack> consumer) {
        return new NonNullList_1_20_R2(size, consumer);
    }

    @Override
    public FakeEntity generateFakeEntity(Location location, EntityType type, Object data) {
        return new FakeEntity_1_20_R2(location, type, data);
    }
