...
public interface WorldGuardCompatibility {
    ...
    /**
     * Tests the state flag with the given <code>flagName</code>. This method will return
     * <code>true</code> if that flag is enabled for the given <code>location</code>.
     *
     * <p>
     * If the <code>player</code> argument is not <code>null</code>, this method can also return
     * <code>true</code> if they have permission to override a region's permission. For example, region
     * owners can bypass most, if not all restrictions.
     *
    ...
    /**
     * Returns the value of a flag with the given <code>flagName</code>. The returned value depends on
     * the region that contains the <code>location</code>.
     *
     * <p>
     * The datatype of the returned value depends on the {@link FlagType}.
     *
     * @param location The non-null coordinates to test for regions.
     * @param flagName The non-null name of the flag.
     * @return The value of the flag for that region, or <code>null</code>.
     */
    @Nullable Object getValue(@NotNull Location location, @NotNull String flagName);
    ...
    /**
     * Returns <code>true</code> if world guard is installed. This is the same as
     * <code>PluginManager#getPlugin("WorldGuard") != null</code>.
     *
    ...
    /**
     * Returns a keySet from the {@link java.util.Map} of register flags. Only flags added using
     * {@link #registerFlag(String, FlagType)} are added to the backing map.
     *
     * @return The non-null set of flag names.
     */
    @NotNull Set<String> getRegisteredFlags();
