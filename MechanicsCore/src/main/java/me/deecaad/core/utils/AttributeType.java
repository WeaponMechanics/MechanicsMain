 ...
 *
 * Attributes have a UUID, string identifier, minecraft name, value, and slot.
 * The UUID should be completely unique, which is why most people say to use a
 * random attribute. If the UUID is not unique, if 2 items with the same uuid
 * exist, one of the effects will be ignored. The string identifier is
 * basically unused by minecraft, so it can be anything. The slot should usually
 * be defined, so people don't hold armor to gain its benefits. Use <code>null</code>
 * for any slot.
 *
 * <p>MechanicsCore modifies the least significant bits of the UUID when the
 * slot is defined. HAND gets +1, HEAD +2, CHEST +3, LEGS +4, FEET +5. This
 * ensures the attributes don't override each other.
 */
