...
public class Block_1_15_R1 implements BlockCompatibility {
    ...
    private PacketPlayOutBlockChange getBlockMaskPacket(Block bukkitBlock, IBlockData mask) {
        ...
        PacketPlayOutBlockChange packet = new PacketPlayOutBlockChange(world.c(x, z), position);
        packet.block = mask;

        return packet;
    }

    public Object getMultiBlockMaskPacket(List<Block> blocks, @Nullable org.bukkit.Material mask, byte data) {
        if (blocks == null || blocks.size() <= 0) {
            throw new IllegalArgumentException("No blocks are being changed!");
        }

        return getMultiBlockMaskPacket(blocks, mask == null ? null : ((CraftBlockData) mask.createBlockData()).getState());
    }
    ...
    public Object getMultiBlockMaskPacket(List<Block> blocks, @Nullable BlockState mask) {
        ...
        return getMultiBlockMaskPacket(blocks, mask == null ? null : ((CraftBlockState) mask).getHandle());
    }
    ...
    private PacketPlayOutMultiBlockChange getMultiBlockMaskPacket(List<Block> blocks, @Nullable IBlockData mask) {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            int x = (block.getX() % 16) & 15;
            int y = block.getY();
            int z = (block.getZ() % 16) & 15;

            x = (x << 12);
            z = (z << 8 );

            // Setting the (x, y, z) location into VarInt format
            short location = (short) (x | y | z);
