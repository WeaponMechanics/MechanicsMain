...
import me.deecaad.weaponmechanics.compatibility.v1_9_R2;
...
public class Projectile_1_9_R2 implements IProjectileCompatibility {
    ...
    public void spawnDisguise(CustomProjectile customProjectile, Vector location, Vector motion) {
        ...
        switch (projectileDisguise) {
            case FALLING_BLOCK:
                net.minecraft.server.v1_9_R2.Block nmsBlock = CraftMagicNumbers.getBlock(customProjectile.projectile.getProjectileStack().getType());
                net.minecraft.server.v1_9_R2.IBlockData nmsIBlockData = nmsBlock.getBlockData();

                EntityFallingBlock nmsEntityFallingBlock = new EntityFallingBlock(((org.bukkit.craftbukkit.v1_9_R2.CraftWorld) world).getHandle(), location.getX(), location.getY(), location.getZ(), nmsIBlockData);
                customProjectile.setProjectileDisguiseId(nmsEntityFallingBlock.getId());

                PacketPlayOutSpawnEntity spawn = new PacketPlayOutSpawnEntity(nmsEntityFallingBlock, net.minecraft.server.v1_9_R2.Block.getCombinedId(nmsIBlockData));
            ...
            case DROPPED_ITEM:
                ...
                EntityItem nmsEntityItem = new EntityItem(((org.bukkit.craftbukkit.v1_9_R2.CraftWorld) world).getHandle(), location.getX(), location.getY(), location.getZ(), nmsStack);
                ...
                break;
            default:
                net.minecraft.server.v1_9_R2.Entity nmsEntity = ((org.bukkit.craftbukkit.v1_9_R2.CraftWorld) world).createEntity(location.toLocation(world, yaw, pitch), projectileDisguise.getEntityClass());
    ...
    public void updateDisguise(CustomProjectile customProjectile, Vector location, Vector motion, Vector lastLocation) {
        ...
        if (motionLength > 8 || motionLength == 0) {
            net.minecraft.server.v1_9_R2.Entity nmsEntity = (net.minecraft.server.v1_9_R2.Entity) customProjectile.projectileDisguiseNMSEntity;
    ...
    public HitBox getHitBox(org.bukkit.block.Block block) {
        ...
        net.minecraft.server.v1_9_R2.Block nmsBlock = blockData.getBlock();

        // Passable block check -> false means passable (thats why !)
        if (!(blockData.d(worldServer, blockPosition) != net.minecraft.server.v1_9_R2.Block.k && nmsBlock.a(blockData, false))) return null;
